...
`);
  ...
  parts.push(
    table(
      'hub_parts',
      ['id', 'category', 'name', 'brand', 'img_url', 'images', 'stars_calification',
  ...
  parts.push(
    table(
      ...
      ['id', 'seller_name', 'title', 'description', 'images', 'price', 'location',
       'category', 'condition', 'is_featured', 'spec_year', 'spec_mileage', 'spec_hp',
      ...
      m.HUB_MARKET_ITEMS_MOCK.map((i) => [
        ...
        sql(i.price), sql(i.location), `${sql(i.category)}::public.hub_market_category`,
  ...
  parts.push(
    table(
      'news',
      ['id', 'title', 'text', 'text_large', 'image_url', 'images', 'scope', 'author', 'published_at'],
      m.NEWS_MOCK.map((n) => [
